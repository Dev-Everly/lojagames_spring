...
@Service
	public class UsuarioService {
		...
		public Optional<Usuario> cadastrarUsuario(Usuario usuario) {
	        ...
	        // 1. Valida se a data foi informada e se tem mais de 18 anos
	        if (usuario.getDataNascimento() == null || Period.between(usuario.getDataNascimento(), LocalDate.now()).getYears() < 18) {
	            throw new ResponseStatusException(
	            		HttpStatus.BAD_REQUEST, "O usuário deve possuir idade igual ou superior a 18 anos.");
	        }

	        // 2. Valida se o usuário já existe
	        if (usuarioRepository.findByUsuario(usuario.getUsuario()).isPresent()) {
	            throw new ResponseStatusException(
	            		HttpStatus.BAD_REQUEST, "O usuário já está cadastrado!");
	        }
		...
		public Optional<Usuario> atualizarUsuario(Usuario usuario) {
			...
			if(usuarioExistente.isPresent() && !usuarioExistente.get().getId().equals(usuario.getId()))
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"O e-mail já existe!",null);
